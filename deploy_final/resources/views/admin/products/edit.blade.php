...
@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.products.index') }}" class="inline-flex items-center gap-1 text-sm font-semibold text-ink-500 hover:text-honey-600">
    ...
    <div class="w-full max-w-xl rounded-2xl border-2 border-ink-200 bg-cream-100 p-6 card-brutal-hover">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
            ...
            <label class="flex flex-col gap-1.5">
                ...
                @error('category_id') <span class="text-xs font-bold text-honey-600">{{ $message }}</span> @enderror
            </label>
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <textarea name="description" rows="3"
                    class="rounded-xl border-2 border-ink-200 bg-cream-50 px-4 py-3 text-sm outline-none transition focus:border-honey-500 focus:ring-2 focus:ring-honey-500/20">{{ old('description', $product->description) }}</textarea>
            </label>
            ...
            <div>
                ...
                <div class="mt-2 flex gap-4">
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="ready" @checked(old('status', $product->status) === 'ready') class="h-4 w-4 accent-honey-500">
                        Ready
                    </label>
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="sold_out" @checked(old('status', $product->status) === 'sold_out') class="h-4 w-4 accent-honey-500">
                        Sold Out
                    </label>
                </div>
