...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // Semua produk ready untuk section #menu (filter client-side via JS).
        $homeProducts = Product::where('status', 'ready')
            ->with('category')
            ->latest()
            ->get();

        // Semua paket ready untuk section #paket.
        $homeBundles = Bundle::with('items.product')
            ->where('status', 'ready')
            ->latest()
            ->get();

        return view('home', compact('categories', 'heroProducts', 'homeProducts', 'homeBundles'));
    }
