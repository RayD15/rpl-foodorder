        ...
        <form action="{{ route('admin.bundles.update', $bundle) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
                ...
                <div id="bundle-items" class="flex flex-col gap-2">
                    @forelse ($bundle->items as $index => $item)
                        <div class="bundle-item flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                            ...
                            <input type="number" name="items[{{ $index }}][qty]" value="{{ $item->qty }}" min="1" class="bundle-qty w-full sm:w-16 rounded-xl border-2 border-ink-200 bg-cream-50 px-2 py-2.5 text-center text-sm outline-none transition focus:border-honey-500">
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <textarea name="description" rows="3"
                    class="rounded-xl border-2 border-ink-200 bg-cream-50 px-4 py-3 text-sm outline-none transition focus:border-honey-500 focus:ring-2 focus:ring-honey-500/20">{{ old('description', $bundle->description) }}</textarea>
            </label>
            ...
            <div>
                ...
                <div class="mt-2 flex gap-4">
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="ready" @checked($bundle->isReady()) class="h-4 w-4 accent-honey-500">
                        Ready
                    </label>
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="sold_out" @checked(!$bundle->isReady()) class="h-4 w-4 accent-honey-500">
                        Sold Out
                    </label>
                </div>
