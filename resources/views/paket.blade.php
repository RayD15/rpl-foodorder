    ...
    @if ($bundles->count())
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
            @foreach ($bundles as $bundle)
                <a href="{{ route('bundle.show', $bundle['id']) }}" class="group flex flex-col overflow-hidden card-brutal card-brutal-hover">
                    <div class="relative aspect-[4/3] overflow-hidden bg-cream-200">
                        <img src="{{ $bundle['image'] }}" alt="{{ $bundle['name'] }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105">
                        @if ($bundle['savings'] > 0)
                    ...
                    <div class="flex flex-1 flex-col p-3.5">
                        ...
                        <div class="mt-2 flex items-end justify-between gap-2">
                            <div>
                                ...
                                @endif
                                <span class="text-sm font-extrabold text-tomato-600">{{ 'Rp' . number_format($bundle['price'], 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                </a>
