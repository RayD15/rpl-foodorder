...
@section('content')
    <nav class="animate-rise mb-4">
        <a href="{{ route('paket') }}" class="inline-flex items-center gap-1 text-sm font-semibold text-ink-500 hover:text-tomato-600">
    ...
    <div class="animate-rise overflow-hidden rounded-3xl border-2 border-ink-200 bg-cream-100 card-brutal-hover" style="--reveal-delay:60ms">
        <div class="relative aspect-[4/3] bg-cream-200">
            <img src="{{ $bundle->image_url }}" alt="{{ $bundle->name }}" class="h-full w-full object-cover">
            @if ($bundle->savings > 0)
                <span class="absolute left-3 top-3 rounded-full bg-tomato-500 px-3 py-1 text-xs font-extrabold text-white shadow-[2px_2px_0_0_var(--color-ink-900)]">
        ...
        <div class="p-6">
            ...
            <div class="mt-1 flex items-baseline gap-2">
                ...
                @endif
                <span class="text-2xl font-extrabold text-tomato-600">{{ 'Rp' . number_format($bundle->price, 0, ',', '.') }}</span>
            </div>
                ...
                <div class="flex flex-col gap-2">
                    @foreach ($bundle->items as $item)
                        <div class="flex items-center gap-3 rounded-2xl border-2 border-ink-200 bg-cream-50 p-3">
                            <img src="{{ $item->product?->image_url }}" alt="{{ $item->product?->name }}" class="h-12 w-12 flex-shrink-0 rounded-xl object-cover bg-cream-200">
            ...
            <div class="mt-6">
                ...
                <div class="inline-flex items-center gap-4 rounded-2xl border-2 border-ink-200 bg-cream-50 p-2">
                    ...
                    <button type="button" id="qty-inc" class="grid h-10 w-10 place-items-center rounded-xl bg-tomato-500 text-lg font-bold text-white shadow-[2px_2px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">+</button>
                </div>
            </div>

            <div class="mt-6 grid grid-cols-2 gap-3">
                <button type="button" id="bundle-add-btn" data-add-bundle="{{ $bundle->id }}" data-qty="1"
                    class="col-span-2 inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-tomato-500 px-6 py-4 text-base font-extrabold text-white shadow-[5px_5px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[3px_3px_0_0_var(--color-ink-900)] active:translate-x-1 active:translate-y-1 active:shadow-none">
                ...
                <a href="{{ route('paket') }}"
                    class="col-span-2 flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-cream-100 px-6 py-3.5 text-sm font-extrabold text-ink-800 shadow-[3px_3px_0_0_var(--color-ink-900)] transition hover:bg-cream-200 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
                    Lanjut ke Menu
            ...
            <div class="mt-6 rounded-2xl border-2 border-ink-200 bg-cream-50 p-4 text-center">
                ...
                <a href="{{ route('paket') }}"
                    class="mt-3 inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-tomato-500 px-6 py-3 text-sm font-extrabold text-white shadow-[3px_3px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
                    Lihat Paket Lain
                </a>
            </div>
            @endif
        ...
        <section class="reveal mt-9">
            ...
            <div class="grid grid-cols-2 gap-4">
                @foreach ($related as $b)
                    <a href="{{ route('bundle.show', $b) }}" class="group flex flex-col overflow-hidden card-brutal card-brutal-hover">
                        <div class="relative aspect-[4/3] overflow-hidden bg-cream-200">
                            <img src="{{ $b->image_url }}" alt="{{ $b->name }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105">
                            @if ($b->savings > 0)
                                <span class="absolute left-2 top-2 rounded-full bg-tomato-500 px-2.5 py-1 text-xs font-extrabold text-white shadow-[2px_2px_0_0_var(--color-ink-900)]">
                        ...
                        <div class="flex flex-1 flex-col p-3.5">
                            ...
                            <div class="mt-2 flex items-end justify-between gap-2">
                                <div>
                                    ...
                                    @endif
                                    <span class="text-sm font-extrabold text-tomato-600">{{ 'Rp' . number_format($b->price, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
