...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-1 font-display text-3xl font-normal text-ink-900">Tambah Produk</h1>
    </div>
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <textarea name="description" rows="3"
                    class="rounded-xl border-2 border-ink-200 bg-cream-50 px-4 py-3 text-sm outline-none transition focus:border-tomato-500 focus:ring-2 focus:ring-tomato-500/20">{{ old('description') }}</textarea>
            </label>
            ...
            <div>
                ...
                <div class="mt-2 flex gap-4">
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="ready" checked class="h-4 w-4 accent-tomato-500">
                        Ready
                    </label>
                    <label class="flex items-center gap-2 text-sm font-bold text-ink-800">
                        <input type="radio" name="status" value="sold_out" class="h-4 w-4 accent-tomato-500">
                        Sold Out
                    </label>
                </div>
