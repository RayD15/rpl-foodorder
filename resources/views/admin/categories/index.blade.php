...
@section('content')
    ...
    <div class="max-w-xl rounded-2xl border-2 border-ink-200 bg-cream-100 p-6 card-brutal-hover">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="flex flex-col gap-3 sm:flex-row sm:gap-3">
            ...
            <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama kategori baru (mis. Makanan)"
                class="flex-1 rounded-xl border-2 border-ink-200 bg-cream-50 px-4 py-3 text-sm outline-none transition focus:border-tomato-500 focus:ring-2 focus:ring-tomato-500/20">
            <button type="submit" class="rounded-xl border-2 border-ink-900 bg-tomato-500 px-5 py-3 text-sm font-extrabold text-white shadow-[3px_3px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none sm:w-auto">+ Tambah</button>
        </form>
        @error('name') <p class="mt-2 text-xs font-bold text-tomato-600">{{ $message }}</p> @enderror
    </div>
    ...
    <div id="edit-category-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-sm rounded-2xl border-2 border-ink-900 bg-cream-100 p-6 shadow-[6px_6px_0_0_var(--color-ink-900)]">
            ...
            <form id="edit-category-form" method="POST" class="flex flex-col gap-3">
                ...
                <div class="flex gap-2">
                    <button type="button" id="edit-category-cancel" class="flex-1 rounded-xl border-2 border-ink-900 bg-cream-100 py-3 text-sm font-extrabold text-ink-800 shadow-[2px_2px_0_0_var(--color-ink-900)] transition hover:bg-cream-200 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">Batal</button>
                    <button type="submit" class="flex-1 rounded-xl border-2 border-ink-900 bg-tomato-500 py-3 text-sm font-extrabold text-white shadow-[2px_2px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">Simpan</button>
                </div>
            </form>
        </div>
    </div>
