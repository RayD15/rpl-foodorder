    ...
    <div class="max-w-xl rounded-2xl border-2 border-ink-200 bg-cream-100 p-6 card-brutal-hover">
        <form action="{{ route('admin.settings.update') }}" method="POST" class="flex flex-col gap-4">
            ...
            <div>
                ...
                @error('whatsapp_number') <p class="mt-2 text-xs font-bold text-tomato-600">{{ $message }}</p> @enderror
            </div>

            <button type="submit"
                class="mt-2 inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-tomato-500 px-6 py-3.5 text-sm font-extrabold text-white shadow-[4px_4px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
