...
@section('content')
        ...
        <form id="checkout-form" class="flex flex-col gap-4">
            ...
            <div class="mt-1 rounded-2xl border-2 border-dashed border-ink-300 bg-cream-50 p-4 text-sm">
                ...
                <div class="flex justify-between pt-3 text-base font-extrabold text-ink-900">
                    <span>Total</span>
                    <span id="checkout-total" class="text-tomato-600"></span>
                </div>
            </div>

            <button type="submit"
                class="mt-2 inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-tomato-500 px-6 py-5 text-base font-extrabold text-white shadow-[5px_5px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-[3px_3px_0_0_var(--color-ink-900)] active:translate-x-1 active:translate-y-1 active:shadow-none">
    ...
    <div id="checkout-confirm-modal"
        class="fixed inset-0 z-50 hidden items-end justify-center bg-black/50 p-4 sm:items-center sm:p-0"
        aria-hidden="true">
        <div class="animate-rise w-full max-w-md rounded-3xl border-2 border-ink-200 bg-cream-100 p-6 shadow-2xl"
            style="--reveal-delay:0ms">
            <h2 class="font-display mb-4 text-2xl font-normal text-ink-900">Konfirmasi Pesanan</h2>

            <div class="mb-4 max-h-56 space-y-2 overflow-y-auto divide-y divide-dashed divide-ink-200">
                <div class="flex justify-between text-sm text-ink-800">
                    <span>Nama</span><span id="confirm-name" class="font-bold"></span>
                </div>
                <div class="flex justify-between text-sm text-ink-800">
                    <span>Kelas</span><span id="confirm-class" class="font-bold"></span>
                </div>
                <div class="flex justify-between text-sm text-ink-800">
                    <span>Catatan</span><span id="confirm-notes" class="font-bold italic"></span>
                </div>
            </div>
            ...
            <div class="mb-4 text-sm">
                ...
                <div class="flex justify-between pt-3 text-base font-extrabold text-ink-900">
                    <span>Total</span>
                    <span id="confirm-total" class="text-tomato-600"></span>
                </div>
            </div>

            <div class="flex gap-3">
                <button id="confirm-cancel" type="button"
                    class="flex-1 rounded-xl border-2 border-ink-200 bg-cream-50 px-4 py-3 text-sm font-bold text-ink-700 transition hover:bg-ink-100">
                    Batal
                </button>
                <button id="confirm-submit" type="button"
                    class="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-ink-900 bg-tomato-500 px-4 py-3 text-sm font-extrabold text-white shadow-[3px_3px_0_0_var(--color-ink-900)] transition hover:bg-tomato-600 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
